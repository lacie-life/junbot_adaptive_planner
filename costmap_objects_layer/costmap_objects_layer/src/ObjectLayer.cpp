...
PLUGINLIB_EXPORT_CLASS(object_layer::ObjectLayer, costmap_2d::Layer)

using costmap_2d::LETHAL_OBSTACLE;

static const std::string tag {"[OBJECT-LAYER] "};
...
namespace object_layer
{
    ...
    void ObjectLayer::onInitialize()
    {
        ...
        dsrv_ = std::make_shared<dynamic_reconfigure::Server<costmap_objects_layer::ObjectLayerConfig>>(nh);
        dynamic_reconfigure::Server<costmap_objects_layer::ObjectLayerConfig>::CallbackType cb = boost::bind(
    ...
    void ObjectLayer::parseTopicsFromYaml(ros::NodeHandle &nh, const std::string &param)
    {
        ...
        if (nh.getParam(param, param_yaml))
        {
            ...
            for (std::size_t i = 0; i < param_yaml.size(); ++i)
            {
                ...
                else
                {
                    ...
                    {
                        _subs.push_back(nh.subscribe(topic_name, 100, &ObjectLayer::zoneCallback, this));
                    }
                    else if (param.compare("obstacle_topics") == 0)
                    {
                        _subs.push_back(nh.subscribe(topic_name, 100, &ObjectLayer::obstaclesCallback, this));
                    }
    ...
    void ObjectLayer::parseFormListFromYaml(const ros::NodeHandle &nh, const std::string &param)
    {
        XmlRpc::XmlRpcValue param_yaml;
        if (nh.getParam(param, param_yaml))
        {
    ...
    void ObjectLayer::convert(const XmlRpc::XmlRpcValue &val, geometry_msgs::Point &point)
    {
        try
        {
    ...
    bool ObjectLayer::robotInZone(const Polygon &zone)
    {
        if (!_one_zone_mode)
        {
    ...
    void ObjectLayer::reconfigureCB(costmap_objects_layer::ObjectLayerConfig &config, uint32_t level)
    {
                                  ...
                                  int max_j)
    {
        ...
        {
            std::cout << "Polygon " << i << "\n";
            std::cout << _obstacle_polygons[i].size() << "\n";
            try {
    ...
    void ObjectLayer::computeMapBounds()
    {
        std::lock_guard<std::mutex> l(_data_mutex);
    ...
    void ObjectLayer::raytrace(int x0, int y0, int x1, int y1, std::vector<PointInt> &cells)
    {
        int dx = abs(x1 - x0);
        int dy = abs(y1 - y0);
        PointInt pt;
    ...
    void ObjectLayer::zoneCallback(const custom_msgs::ZoneConstPtr &zone_msg)
    {
    ...
    void ObjectLayer::obstaclesCallback(const custom_msgs::ObstaclesConstPtr &obstacles_msg)
    {
        if (_clear_obstacles)
        {
...
//                _obstacle_classId.push_back(obstacles_msg->list[i].id);
    ...
    geometry_msgs::Point ObjectLayer::getRobotPoint()
    {
